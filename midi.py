...
def create_MIDI_test(music, outfile='output.mid', outdir='midifiles'):
    ...
    for tracksn, tracks in enumerate(music.sections):

        for trackn, track in enumerate(tracks):
            if track in drum_tracks: continue
            print(trackn, track.instrument)
            mf.addProgramChange(0, trackn, 0, track.instrument)
            time = tracksn * 10 # FIXME get correct multiplier 
